...
export class RoomService {
  ...
  //   socketIdToUserId: Map<string, string>,
  // ) {
  //   // 모든 소켓에서 userId가 일치하는 socketId를 찾아 모든 방에서 제거
  //   for (const [socketId, uid] of socketIdToUserId.entries()) {
  //     if (uid === userId) {
...
//   // ready 상태 관리용 Map 추가
//   private readyMap: Map<string, Set<string>> = new Map(); // roomId -> Set<userId>
...
//     if (!this.readyMap.has(roomId)) {
//       this.readyMap.set(roomId, new Set());
//     }
//     this.readyMap.get(roomId)!.add(userId);
//     this.logger.log(`[setReady] userId=${userId}, roomId=${roomId}`);
//   }
...
//     const allReady = readySet && users.every((uid) => readySet.has(uid));
//     this.logger.log(
//       `[canStart] roomId=${roomId}, allReady=${allReady}, users=${users.join(',')}, ready=${readySet ? Array.from(readySet).join(',') : ''}`,
//     );
//     return !!allReady;
//   }
